...
module.exports = function() {
  ...
  // TODO use D3.d3.rebind to obtain this from 'super class'
...
module.exports = function(d3_svg_line, d3_select) {
  ...
  function PathArea() {
    var d3Area = D3.d3.svg.area().interpolate('monotone');
    ...
    plot: {
      ...
      ichimoku: function(data, accessor) {
        ...
        return D3.d3.scale.linear()
          .domain(D3.d3.extent(values).map(widen(0.02)))
          .range([1, 0]);
      },
      ...
      ohlc: function (data, accessor) {
        ...
        return D3.d3.scale.linear()
          .domain([D3.d3.min(data.map(accessor.low())), D3.d3.max(data.map(accessor.high()))].map(widen(0.02)))
          .range([1, 0]);
      },
      ...
      volume: function (data, accessor) {
        ...
        return D3.d3.scale.linear()
          .domain([0, D3.d3.max(data.map(accessor))*1.15])
          .range([1, 0]);
      },
      ...
      atrtrailingstop: function (data, accessor) {
        ...
        return D3.d3.scale.linear().domain(D3.d3.extent(values).map(widen(0.04)))
          .range([1, 0]);
      },

      rsi: function () {
        return D3.d3.scale.linear().domain([0, 100])
          .range([1, 0]);
      },
      ...
      aroon: function () {
        return D3.d3.scale.linear().domain([-100, 100])
          .range([1, 0]);
      },
      ...
      bollinger: function (data, accessor) {
         ...
         return D3.d3.scale.linear()
              .domain([
                 D3.d3.min(data.map(function(d){return accessor.lower(d);})),
                 D3.d3.max(data.map(function(d){return accessor.upper(d);}))
                ].map(widen(0.02)))
              .range([1, 0]);
      }
    },
